...
const { StatusCodes: { OK, CREATED, SERVICE_UNAVAILABLE, BAD_REQUEST }, } = require('http-status-codes');
...
const userGoogleAuthentication = async (request, response, next) => {
    try {
        ...
        const isUserAvailable = await UserDataModel.findOne({ email });
        ...
        if (!isUserAvailable) {
            ...
            response.status(OK).json({
                error: false, statusCode: OK, message: "Login Successful", data: isUserAvailable
            })
